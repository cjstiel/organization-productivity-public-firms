...
#
# -----------------------------------------------------------------------------------------------
# content: This program repeats the analysis for the 'electricity and gas' sample but includes a 
# time trend in production function. (see Section A.3.3 in the supplementary material online).
# version: 171
#================================================================================================
...
boot.acf <- function(data,indices,method){
  ...
  instr <<- cbind(rep(1,nrow(data_bp)),data_bp$l_m,lag(data_bp$f_m),data_bp$k_m,data_bp$l_m^2
                  ...
                  ,data_bp$l_m*data_bp$t,lag(data_bp$f_m)*data_bp$t,data_bp$k_m*data_bp$t
                  ,lag(data_bp$sn)*lag(data_bp$defl_sn)
                  ,lag(data_bp$ga)*lag(data_bp$defl_ga)
                  ,lag(data_bp$sa)*lag(data_bp$p_sa_log)
                  ,data_bp$se_water*log(100)
...
Inputs_gmm_all <- as.matrix(cbind(rep(1,nrow(data_gmm)),data_gmm$l_m,data_gmm$f_m,data_gmm$k_m
                                  ,0.5*(data_gmm$l_m)^2,0.5*(data_gmm$f_m)^2,0.5*(data_gmm$k_m)^2
                                  ,data_gmm$l_m*data_gmm$t,data_gmm$f_m*data_gmm$t,data_gmm$k_m*data_gmm$t
                          	  ,data_gmm$sn*data_gmm$defl_sn
                          	  ,data_gmm$ga*data_gmm$defl_ga
                          	  ,data_gmm$sa*data_gmm$p_sa_log
                          	  ,data_gmm$se_water*log(100)
...
instr <- cbind(rep(1,nrow(data_p)),data_p$l_m,lag(data_p$f_m),data_p$k_m
               ...
               ,data_p$l_m*data_p$t,lag(data_p$f_m)*data_p$t,data_p$k_m*data_p$t
...
betas2 <- rbind(optimization$p1[j],optimization$p2[j],optimization$p3[j],optimization$p4[j]
		...
		,optimization$p17[j],optimization$p18[j],optimization$p19[j]
		,optimization$p20[j])
...
data_p$elasticity_fdl <- (betas_final[3] + betas_final[6]*Inputs[,3] + betas_final[18]*Inputs[,2] 
                          + betas_final[20]*Inputs[,4] + betas_final[9]*data_p$t)


# Calculate output elasticity for capital
# ---------------------------------------
# elasticity_k = b_k + b_kk*k + b_lk*l + b_kf*f + b_kt*t
data_p$elasticity_cap <- (betas_final[4] + betas_final[7]*Inputs[,4] + betas_final[19]*Inputs[,2] 
                          + betas_final[20]*Inputs[,3] + betas_final[10]*data_p$t)
