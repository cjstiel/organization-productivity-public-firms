...
### 
### Dateiname des Programmcodes: Integriert_Schaetzung_20181213_v183_cs.R
### erstellt: 13.12.2018
### von: Caroline Stiel
...
####################################################################################################
###
### Version 183:  - Formulierung mit Translog-Modell | Wagebill | Disaggregierte Daten
...
###               - Sensitivitätsanalyse: Zeittrend basic mit SE
#############################  Start Analyse #######################################################
...
#Pfad3<-"Q:\\AFS\\55_FDZ\\Forschungsprojekte\\2012-2094 DIW - Richter (4)\\KDFV\\2018_12_13_cs\\"
...
first_stage_OLS_m1 <- lm(va3_m ~
                        ...
                        + l_m:f_m + l_m:k_m + f_m:k_m
                        # Zeittrend
                        + I(l_m*t) + I(f_m*t) + I(k_m*t) + I(0.5*l_m^2*t) + I(0.5*f_m^2*t) 
                        + I(0.5*k_m^2*t) + I(l_m*f_m*t) + + I(l_m*k_m*t) + + I(k_m*f_m*t) 
                        # Sector controls
...
instr <- cbind(rep(1,nrow(data_p)),data_p$l_m,lag(data_p$f_m),data_p$k_m
               ...
               ,data_p$l_m*data_p$t,lag(data_p$f_m)*data_p$t,lag(data_p$k_m)*data_p$t
               ,data_p$ga*data_p$defl_ga
...
data_gmm$elasticity_lohn <- (betas_final[2] + betas_final[5]*Inputs_gmm_all[,2] 
                           + betas_final[21]*Inputs_gmm_all[,3] 
                           + betas_final[22]*Inputs_gmm_all[,4]
			                     + betas_final[8]*data_gmm$t)
...
data_gmm$elasticity_fdl <- (betas_final[3] + betas_final[6]*Inputs_gmm_all[,3] 
                          + betas_final[21]*Inputs_gmm_all[,2] 
                          + betas_final[23]*Inputs_gmm_all[,4]
			                    + betas_final[9]*data_gmm$t)
...
data_gmm$elasticity_cap <- (betas_final[4] + betas_final[7]*Inputs_gmm_all[,4] 
                          + betas_final[22]*Inputs_gmm_all[,2] 
                          + betas_final[23]*Inputs_gmm_all[,3]
			                    + betas_final[10]*data_gmm$t)
