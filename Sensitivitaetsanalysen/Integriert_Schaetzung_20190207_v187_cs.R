...
### 
### Dateiname des Programmcodes: Integriert_Schaetzung_20190207_v187_cs.R
### erstellt: 07.02.2019
### von: Caroline Stiel
...
####################################################################################################
###
### Version 187: - Formulierung mit Translog-Modell | Wagebill | Disaggregierte Daten
###              - reine Erzeuger (Wärme/Strom) 2003-2014
###              - Sensitivitätsanalyse mit Interactions im LOM mit SE
#############################  Start Analyse #######################################################
...
#Pfad3<-"Q:\\AFS\\55_FDZ\\Forschungsprojekte\\2012-2094 DIW - Richter (4)\\KDFV\\2017_08_18_cs_4\\"
...
gmm_moment_condition <- function(betas){
  ...
  omega_pol <<- cbind(rep(1,n),lag_omega,lag_omega^2,lag_omega^3
                      ,data_gmm$lag1_eigentuemer2
                      ,data_gmm$shareF,data_gmm$shareFEW
                      ,data_gmm$shareF*data_gmm$shareFEW
                      ,data_gmm$shareF*data_gmm$lag1_eigentuemer2
                      ,data_gmm$shareFEW*data_gmm$lag1_eigentuemer2)
  AR1 <<- lm(omega ~ lag_omega + I(lag_omega^2) + I(lag_omega^3) 
             +data_gmm$lag1_eigentuemer2
             + data_gmm$shareF + data_gmm$shareFEW
             + I(data_gmm$shareF*data_gmm$shareFEW)
             + I(data_gmm$shareF*data_gmm$lag1_eigentuemer2) 
             + I(data_gmm$shareFEW*data_gmm$lag1_eigentuemer2))
...
boot.acf <- function(data,indices,method){
  ...
  first_stage_m <<- lm(va3_m ~ l_m + f_m + k_m + I(0.5*l_m^2) +  I(0.5*f_m^2) + I(0.5*k_m^2)
                      + l_m:f_m + l_m:k_m + f_m:k_m + I(se*defl_se_eex) + I(wm_NWG*defl_wm_NWG)
                      + I(se_gas*defl_se_eg) + I(se_coal*defl_se_coal) + I(se_EE2*log(100)) 
                      + suburban 
                      + l_m:k_m:f_m + l_m:I(f_m^2) 
                      + f_m:I(k_m^2) + l_m:I(k_m^2) + f_m:I(l_m^2) + k_m:I(f_m^2) + k_m:I(l_m^2)
  ...
  Inputs <<- as.matrix(cbind(rep(1,nrow(data_bp)),data_bp$l_m,data_bp$f_m,data_bp$k_m
                             ...
                             ,data_bp$se*data_bp$defl_se_eex,data_bp$wm_NWG*data_bp$defl_wm_NWG
                            ,data_bp$se_gas*data_bp$defl_se_eg,data_bp$se_coal*data_bp$defl_se_coal
                            ,data_bp$se_EE2*log(100),data_bp$suburban,data_bp$l_m*data_bp$f_m
  ...
  Inputs_gmm_all <<- as.matrix(cbind(rep(1,nrow(data_gmm)),data_gmm$l_m,data_gmm$f_m,data_gmm$k_m
                                    ...
                                    ,data_gmm$se*data_gmm$defl_se_eex
                                    ,data_gmm$wm_NWG*data_gmm$defl_wm_NWG
                                    ,data_gmm$se_gas*data_gmm$defl_se_eg
                                    ,data_gmm$se_coal*data_gmm$defl_se_coal
                                    ,data_gmm$se_EE2*log(100),data_gmm$suburban
  ...
  lag_Inputs_all <<- as.matrix(cbind(rep(1,nrow(data_bp)),lag(data_bp$l_m),lag(data_bp$f_m)
                                    ...
                                    ,lag(data_bp$se)*lag(data_bp$defl_se_eex)
                                    ,lag(data_bp$wm_NWG)*lag(data_bp$defl_wm_NWG)
                                    ,lag(data_bp$se_gas)*lag(data_bp$defl_se_eg)
                                    ,lag(data_bp$se_coal)*lag(data_bp$defl_se_coal)
                                    ,lag(data_bp$se_EE2)*log(100),lag(data_bp$suburban)
  ...
  instr <<- cbind(rep(1,nrow(data_bp)),data_bp$l_m,lag(data_bp$f_m),data_bp$k_m,data_bp$l_m^2
                  ,lag(data_bp$f_m)^2,data_bp$k_m^2,data_bp$se*data_bp$defl_se_eex
                  ,data_bp$wm_NWG*data_bp$defl_wm_NWG,data_bp$se_gas*data_bp$defl_se_eg
                  ,data_bp$se_coal*data_bp$defl_se_coal,data_bp$se_EE2*log(100),data_bp$suburban
                  ,data_bp$l_m*lag(data_bp$f_m),data_bp$l_m*data_bp$k_m
                  ,lag(data_bp$f_m)*data_bp$k_m)
  ...
  initial_betas <<- betas_basic_m[c(1:16)]
  betas_fixed <<- 0
...
#                                                                                                  #
# Die Anzahl der geschätzten Parameter aus boot.acf muss in ncol= angepasst werden.                 #
#												                                                                           #
...
clusterBootSE<-function(data,method,B){
  ...
  sterrs <- matrix(NA, nrow=B, ncol=16)
...
data0 <- subset(data0,select=-c(id_ns,id_he1,id_he2,id_he3,id_he4,id_th,id_sh,jab,public
                                ,Lueckenjahre,out_labour,out_lohn,out_int,out_umsatz,out_p_sa
                                ,out_p_LV,out_p_EVU,out_p_TK,out_p_SK,out_p_HH,out_p_VG,out_p_BC))
...
data0$se_EE2 <- ifelse(data0$se_EE==1 | data0$se_water==1 | data0$se_bio==1 | data0$se_sonst==1
                       | data0$se_waste==1,1,0)
...
# 9-Steller, GP09-351115300 Elektrischer Strom, Börsenpreis
# Nimm für 2003 und 2004 'Elektrischer Strom an Weiterverteiler, da noch kein Börsenpreis verfügbar
defl_se_eex <- cbind("Jahr"=seq(2003,2014),"defl_se_eex"=log(c(63.9,69,88,119.9,121.7,151.5,111.5
                                                               ,100,114.4,98.6,79.9,70.5)))
...
data0 <- merge(data0,defl_se_eex,by="Jahr")
...
# Kreiiere den gewichteten PPI 'se_coal'.
# ---------------------------------------
# Da Braunkohlekraftwerke und Steinkohle-Kraftwerke gleich oft vorkommen im Sample, gewichte mit 0.5
data0$defl_se_coal <- 0.5*data0$defl_se_hc + 0.5*data0$defl_se_bk
...
# Gibt es Kraftwerke, die pro Jahr doppelt beobachtet werden?
...
# Wahl des Subsets
# ----------------
# Nimm das subset der reinen Erzeuger (Strom-/Wärme)
data0 <- subset(data0,(se == 1 | wm==1) & ga==0 & wa==0 & sa==0 & sn==0)
...
# Grundmodell mit Controls in t
# -----------------------------
out1_OLS1 <- lm(shareF ~  t + I(t^2) 
                # andere Reorganisation-Variablen
                + eigentuemer2 + shareFEW
                # Unternehmensgröße und Produktionsprozess
                + size_med + size_large + inv_int + wage
                # Sektoren und Kundenstruktur
                + se_gas + se_oil + se_hc + se_waste + se_bio + se_EE + se_water + se_sonst
                # Environment
                + suburban + rurald + rurals + ShareHH + ShareWV_w
                ,data_p)
summary(out1_OLS1)
...
out1_OLS2 <- lm(shareF ~  t + I(t^2)
                # andere Reorganisation-Variablen
                + lag(data_p$eigentuemer2) + lag(data_p$shareFEW)
                # Unternehmensgröße und Produktionsprozess
                + size_med + size_large + lag(data_p$inv_int) + lag(data_p$wage)
                # Sektoren und Kundenstruktur
                + se_gas + se_oil + se_hc + se_waste + se_bio + se_EE + se_water + se_sonst
                # Environment
                + suburban + rurald + rurals + lag(data_p$ShareHH) + lag(data_p$ShareWV_w)
                ,data_p)
summary(out1_OLS2)
...
out2_OLS1 <- lm(shareFEW ~  t + I(t^2)
                ...
                + size_med + size_large + inv_int + wage
                # Sektoren und Kundenstruktur
                + se_gas + se_oil + se_coal + se_waste + se_bio + se_EE + se_water + se_sonst
                # Environment
                + suburban + rurald + rurals + ShareHH + ShareWV_w
                ,data_p)
summary(out2_OLS1)
...
out2_OLS2 <- lm(shareFEW ~  t + I(t^2) 
                # andere Reorganisation-Variablen
                + lag(data_p$eigentuemer2) + lag(data_p$shareF)
                # Unternehmensgröße und Produktionsprozess
                + size_med + size_large + lag(data_p$inv_int) + lag(data_p$wage)
                # Sektoren und Kundenstruktur
                + se_gas + se_oil + se_coal + se_waste + se_bio + se_EE + se_water + se_sonst
...
# Kontrolliere für die multiproduct structure und die Siedlungsstruktur.
# value-added_it = beta_l * l_it + beta_s * s_it + beta_k * k_it  + beta_ll * l_it^2 
#                  + beta_ss * s_it^2  + beta_kk * k_it^2 + 0.5 * beta_ls * lit * s_it 
#                  + 0.5 * b_lk * l_it * k_it + 0.5 * b_ks * k_it * s_it
#
#                  + gamma_1 * wm_HH_it * defl_wm_HH + gamma_2 * wm_NWG_it * defl_wm_NWG 
#                  + gamma_3 * se_gas_it * p_eg
#                  + gamma_5 * se_EE2_it * log(100) 
#
#                  + gamma_7 * suburban_it 
#
#                  + w_it + u_it
...
# Grundmodell in Logs, medianbereinigt und mit allen Sektor-/fuel Covariates (Preise) MIT eex
# ---------------------------------------------------------------------------------------------
first_stage_OLS_m1 <- lm(va3_m ~
                         ...
                         # Sector controls
                         + I(se*defl_se_eex) + I(wm_HH*defl_wm_HH) + I(wm_NWG*defl_wm_NWG) 
                         # Fuel types
                         + I(se_gas*p_eg) + I(se_oil*p_oil) + I(se_hc*p_hc) 
...
# Grundmodell in Logs, medianbereinigt und mit allen Sektor-/fuel Covariates (Preise) OHNE eex
# ---------------------------------------------------------------------------------------------
first_stage_OLS_m3 <- lm(va3_m ~
                           # Inputs der Produktionsfunktion:
                           l_m + f_m + k_m + I(0.5*l_m^2) +  I(0.5*f_m^2) + I(0.5*k_m^2) 
                         + l_m:f_m + l_m:k_m + f_m:k_m
                         # Sector controls
                         ...
                         + suburban + rurald + rurals
                         ,data_p)
summary(first_stage_OLS_m3)


# Grundmodell in Logs, medianbereinigt mit Fuel-DEFLATOREN anstelle von Preisen OHNE eex
# -------------------------------------------------------------------------------------------
first_stage_OLS_m4 <- lm(va3_m ~
                           # Inputs der Produktionsfunktion:
                           l_m + f_m + k_m + I(0.5*l_m^2) +  I(0.5*f_m^2) + I(0.5*k_m^2) 
                         + l_m:f_m + l_m:k_m + f_m:k_m
                         # Sector controls
                         ...
                         + suburban + rurald + rurals
                         ,data_p)
summary(first_stage_OLS_m4)


# Grundmodell in Logs, medianbereinigt mit Fuel-DEFLATOREN anstelle von Preisen MIT eex
# -------------------------------------------------------------------------------------------
first_stage_OLS_m5 <- lm(va3_m ~
                           # Inputs der Produktionsfunktion:
                           l_m + f_m + k_m + I(0.5*l_m^2) +  I(0.5*f_m^2) + I(0.5*k_m^2) 
                         + l_m:f_m + l_m:k_m + f_m:k_m
                         # Sector controls
                         ...
                         + suburban + rurald + rurals
                         ,data_p)
summary(first_stage_OLS_m5)



# Fasse se_bio, se_water, se_EE und se_sonst in se_EE2 zusammen, da es sonst zu wenig
# Beobachtungen in den einzelnen Kategorien beim Bootstrap-Sampling gibt.
# Fasse auch bk und hc zusammen und multipliziere mit gewichtetem PPI.
...
first_stage_OLS_m2 <- lm(va3_m ~
                         ...
                         + l_m:f_m + l_m:k_m + f_m:k_m
                         # Sector controls
                         + I(se*defl_se_eex) + I(wm_NWG*defl_wm_NWG) 
                         # Fuel types
                         + I(se_gas*defl_se_eg) + I(se_coal*defl_se_coal) + I(se_EE2*log(100))
                         # Siedlungscontrols
                         + suburban
...
# First stage OLS estimation in ACF (2005). Eliminiert iid error epsilon_it.
...
Inputs <- as.matrix(cbind(rep(1,nrow(data_p)),data_p$l_m,data_p$f_m,data_p$k_m,0.5*(data_p$l_m)^2
                          ...
                          ,data_p$se*data_p$defl_se_eex
                          ,data_p$wm_NWG*data_p$defl_wm_NWG
                          ,data_p$se_gas*data_p$defl_se_eg,data_p$se_coal*data_p$defl_se_coal
...
# Erstelle Datensatz für die GMM: data_gmm
# ----------------------------------------
...
# Inputs in der GMM-Funktion at time t
# ------------------------------------
# Gnereriere für die Berechnung von omega in der GMM-Funktion und für die spätere Auswertung im 
# AR(1)-Prozess die vollständige Input-Matrix auf gmm-Basis.
# At time t
...
# Inputs in der GMM-Funktion at time t-1
# --------------------------------------
...
# Fixed Inputs at time t
# ----------------------
# Wähle die fixed inputs aus, für die in der 2nd stage keine neuen Koeffizienten berechnet werden.
Inputs_fixed <- as.matrix(rep(0,nrow(data_gmm)))
dim(Inputs_fixed)


# Lagged fixed Inputs at time t-1
# -------------------------------
lag_Inputs_fixed <- as.matrix(rep(0,nrow(data_gmm)))
lag_Inputs_fixed <- na.omit(lag_Inputs_fixed)
...
# Instrumente Z in der GMM-Funktion
# ----------------------------------
instr <- cbind(rep(1,nrow(data_p)),data_p$l_m,lag(data_p$f_m),data_p$k_m,data_p$l_m^2
                ,lag(data_p$f_m)^2,data_p$k_m^2
                ,data_p$se*data_p$defl_se_eex
                ,data_p$wm_NWG*data_p$defl_wm_NWG
                ,data_p$se_gas*data_p$defl_se_eg
                ,data_p$se_coal*data_p$defl_se_coal
                ,data_p$se_EE2*log(100)
                ,data_p$suburban
...
# Modell: alle Koeffizienten in der 2nd stage
# -------------------------------------------
...
betas2 <- rbind(optimization$p1[j],optimization$p2[j],optimization$p3[j],optimization$p4[j]
                 ...
                 ,optimization$p13[j],optimization$p14[j],optimization$p15[j]
                 ,optimization$p16[j])


#########################################################
##  4.6 Bootstrapping der Standardfehler               ##
#########################################################
...
data_gmm$omega2 <- data_gmm$Phi - Inputs_gmm%*%betas_final
...
data_p$elasticity_lohn <- (betas_final[2] + betas_final[5]*Inputs[,2] 
                           + betas_final[14]*Inputs[,3] + betas_final[15]*Inputs[,4])


# Output elasticity for fremde DL
# -------------------------------
# elasticity_v = b_f + b_ff*f + b_lf*l + b_kv*k
data_p$elasticity_fdl <- (betas_final[3] + betas_final[6]*Inputs[,3] + betas_final[14]*Inputs[,2] 
                          + betas_final[16]*Inputs[,4])


# Output elasticity for capital
# -----------------------------
# elasticity_k = b_k + b_kk*k + b_lk*l + b_kf*f
data_p$elasticity_cap <- (betas_final[4] + betas_final[7]*Inputs[,4] + betas_final[15]*Inputs[,2] 
                          + betas_final[16]*Inputs[,3])
...
# ---------------------------------
# elasticity_l = b_l + b_ll*l + b_lf*f + b_kl*k
data_gmm$elasticity_lohn <- (betas_final[2] + betas_final[5]*Inputs_gmm[,2] 
                             + betas_final[14]*Inputs_gmm[,3] + betas_final[15]*Inputs_gmm[,4])


# Output elasticity for fremde DL
# -------------------------------
# elasticity_v = b_f + b_ff*f + b_lf*l + b_kv*k
data_gmm$elasticity_fdl <- (betas_final[3] + betas_final[6]*Inputs_gmm[,3] 
                            + betas_final[14]*Inputs_gmm[,2] + betas_final[16]*Inputs_gmm[,4])


# Output elasticity for capital
# -----------------------------
# elasticity_k = b_k + b_kk*k + b_lk*l + b_kf*f
data_gmm$elasticity_cap <- (betas_final[4] + betas_final[7]*Inputs_gmm[,4] 
                            + betas_final[15]*Inputs_gmm[,2] + betas_final[16]*Inputs_gmm[,3])
...
# Hier können nur die Firmen untersucht werden, für die das lag t-1 beobachtet wird, d.h. die im
# data_gmm-Datensatz enthalten sind.
# Ordne jeder Firma ihren Produktitivätswert sowie den des Vorjahres zu. Die Omegas müssen im 
# data_gmm-Datensatz erst erstellt werden, da sie bisher nur für den Gesamtdatensatz data_p 
# vorliegen.
data_gmm$lag_omega2 <- data_gmm$lag_Phi - lag_Inputs_gmm%*%betas_final
...
AR1_expost <- plm(omega2 ~ lag_omega2 + I(lag_omega2^2) + I(lag_omega2^3)
                 ...
                 + shareF + shareFEW
                 + I(shareF*shareFEW)
                 + I(shareF*lag1_eigentuemer2) 
                 + I(shareFEW*lag1_eigentuemer2)
...
# ------------------------------------------------
# Is autocorrelation a concern at all?
dwtest(omega2 ~ lag_omega2 + I(lag_omega2^2) + I(lag_omega2^3) 
       + lag1_eigentuemer2 
       + shareF + shareFEW
       + I(shareF*shareFEW)
       + I(shareF*lag1_eigentuemer2) 
       + I(shareFEW*lag1_eigentuemer2),data=data_gmm)
...
# Füge Dummy für die reinen Stromerzeuger ab 2008 ein.
data_p$r_se <- ifelse(data_p$wm==0 & data_p$se==1 & data_p$Eintrittsjahr>=2008,1,0) 
summary(as.factor(data_p$r_se))
...
############################################
##  6.2.2 Regression ohne Zeittrend       ##
############################################
...
explain_pty2 <- plm(omega2 ~ 
                   ...
                   + size_med + size_large
                   # Sector controls
                   + I(wm_HH*defl_wm_HH) + I(wm_NWG*defl_wm_NWG) + I(se*defl_se_eex)
                   # Fuel types
                   + I(se_gas*defl_se_eg) + I(se_oil*defl_se_oil) + I(se_hc*defl_se_hc)
                   + I(se_waste*log(100)) + I(se_bio*log(100))
                   + I(se_EE*log(100)) + I(se_water*log(100)) + I(se_sonst*log(100))
                   ...
                   + suburban + rurald + rurals
                   ,data=data_p
                   ,model="pooling",effect="time",index=c("id"))
summary(explain_pty2)

pdwtest(explain_pty2)
...
# Outsourcing II auf Productivity hat.
linearHypothesis(explain_pty2
                 ,"I(shareFEW * lag1_eigentuemer2)- IshareFEW=0"
                 ,vcov=vcovHC(explain_pty2,method="arellano",cluster=c("group")))
