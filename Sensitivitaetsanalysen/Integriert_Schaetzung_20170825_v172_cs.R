...
### 
### Dateiname des Programmcodes: Integriert_Schaetzung_20170825_v172_cs.R
### erstellt: 25.08.2017
### von: Caroline Stiel
...
####################################################################################################
###
### Version 172:  - Formulierung mit Translog-Modell | Wagebill | Daten auf Unternehmensebene
###               - reine Strom-/Gasfirmen ("liberalised") ohne reine Erzeuger 2003-2014
###		  - Sensitivitätsanalyse ohne reine Gasfirmen mit SE
...
#Pfad3<-"Q:\\AFS\\55_FDZ\\Forschungsprojekte\\2012-2094 DIW - Richter (4)\\KDFV\\2017_08_18_cs_2\\"
...
# -------------------------------------
# 0: rein-öffentlich
# 1: mit privater Minderheitenbeteiligung
data0$eigentuemer2[data0$eigentuemer2==2] <- 0
table(data0$eigentuemer2,data0$Jahr)
...
# -----------------------------------------
data0$eigen <- ifelse(data0$status==2,1,0) 
data0$unlisted <- ifelse(data0$status==3,1,0) 
table(as.factor(data0$unlisted),useNA="ifany")
data0$listed <- ifelse(data0$status==4,1,0)
table(as.factor(data0$listed),useNA="ifany")
data0$privlaw <- ifelse(data0$status==3 | data0$status==4,1,0)
table(as.factor(data0$privlaw),useNA="ifany")
...
# Wahl des Subsets
# ----------------
# Nimm Strom-oder Gasfirmen, die retailen oder verteilen, aber nicht im Wasser- und
# Wärmesektor aktiv sind. Stromerzeugung ist hingegen erlaubt.

# ***************************************************************
# Sensitivitätsanalyse: ohne reine Gasfirmen
data0 <- subset(data0, (wa==0 & wm==0 & (sa==1 | sn==1 | ga==1))
                & (wa==0 & wm==0 & sa==0 & sn==0 & se==0 & ga==1)==FALSE)
# *****************************************************************


# Bereinige Datensatz erneut um konstante Reihen.
# ---------------------------------------
data0 <- subset(data0,select=-c(UK_Code8601,UK_Code8701,B_MBE_EF15_mean,B_MBE_EF17_mean,TM064_b
                                ,UI_Code11_6,wa,wm,wm_NWG,wm_HH,B_waerme_EF1001,B_waerme_EF1010
                                ,B_waerme_EF1011,B_waerme_EF1011a,B_waerme_EF1011b,B_waerme_EF1011c
                                ,B_waerme_EF1011d))
...
out1_OLS1 <- lm(shareF ~ t + I(t^2)
               ...
               # Environment und Demand
               + suburban + rurald + rurals + lag(data_p$ShareTK) + lag(data_p$ShareWV)
               ,data_p)
summary(out1_OLS1)


# Modell mit Interaktion mit Controls in t-1
# ------------------------------------------
...
###############################################################################################
##  4.1 Basismodelle mit OLS ohne ACF-Algorithmus                                            ##
###############################################################################################
...
# Inputs: wagebill, fremde DL, Kapital
...
# Achtung, es gibt im Subsample keine Kraftwerke, die Steinkohle, Braunkohle, Kohle allgemein, oder 
# Müll verbrennen und auch keine EE-Werke und Biomasse-Werke.
...
first_stage_OLS_m1 <- lm(va3_m ~ 
                        ...
                        + I(se_water*log(100)) + I(se_sonst*log(100))
                        # Siedlungscontrols
...
# Grundmodell in Logs, medianbereinigt und nur mit signifikanten Sektor-/fuel covariates und sn, ga
# -------------------------------------------------------------------------------------------------
# Note: Kontrolliere trotzdem weiterhin für sn und ga, da sonst Revenues in diesen Unternehmen
# nicht deflated werden.
...
optimization <- optimx(par=initial_betas,fn=gmm_moment_condition, method=c("BFGS","nlminb","nlm"))
print(optimization)
...
betas2 <- rbind(optimization$p1[j],optimization$p2[j],optimization$p3[j],optimization$p4[j]
                 ...
                 ,optimization$p16[j],optimization$p17[j])
...
data_gmm$elasticity_lohn <- (betas_final[2] + betas_final[5]*Inputs_gmm_all[,2] 
                             + betas_final[15]*Inputs_gmm_all[,3] 
                             + betas_final[16]*Inputs_gmm_all[,4])
...
data_gmm$elasticity_fdl <- (betas_final[3] + betas_final[6]*Inputs_gmm_all[,3] 
                            + betas_final[15]*Inputs_gmm_all[,2] 
                            + betas_final[17]*Inputs_gmm_all[,4])
...
data_gmm$elasticity_cap <- (betas_final[4] + betas_final[7]*Inputs_gmm_all[,4] 
                            + betas_final[16]*Inputs_gmm_all[,2] 
                            + betas_final[17]*Inputs_gmm_all[,3])
...
####################################################################################################
## 	                    6) Auswertung Link between Reorganisation and Productivity                ## 
####################################################################################################
...
# Checking for autocorrelation and heteroscedasticity
# ---------------------------------------------------
...
############################################
##  6.2.3 Regression ohne Zeiteffekten    ##
############################################
...
explain_pty2 <- plm(omega2 ~ 
                   ...
                   # Siedlung
                   + suburban + rurald + rurals
                   ,data=data_p,model="pooling",effect="time")
