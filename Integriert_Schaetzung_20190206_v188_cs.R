...
### 
### Dateiname des Programmcodes: Integriert_Schaetzung_20190205_v188_cs.R
### erstellt: 06.02.2019
### von: Caroline Stiel
...
####################################################################################################
###
### Version 188:  - Formulierung mit Translog-Modell | Wagebill | Daten auf Unternehmensebene
###               - pure water utilities 2003-2014
###               - Grundmodell mit SE
###		  - Interaktion bei Outsourcing Decision
###     - Regress productivity mit log(omega)
###     - Zusammenfassung Eigenbetrieb / Zweckverband auch bei Determinants outsourcing
...
out1_OLS1 <- lm(shareF ~  t + I(t^2) 
		...
		+ lag(data_p$privlaw)  + lag(data_p$eigentuemer2) 
               + lag(data_p$shareFEW)
...
AR1_expost2 <- plm(omega2 ~ lag_omega2 + I(lag_omega2^2) + I(lag_omega2^3)
                   + lag1_unlisted + I(lag1_unlisted*lag1_eigentuemer2) + shareF 
                   + shareFEW
...
dwtest(omega2 ~ lag_omega2 + I(lag_omega2^2) + I(lag_omega2^3) + lag1_unlisted 
...
# Hypothesis Testing (private minority)
# ------------------------------------
# Mit WHITE-ESTIMATOR
# --------------------
# Testet, ob die private Beteiligung bei GmbHs einen signifikanten Einfluss auf die 
# Produktivität hat.
linearHypothesis(AR1_expost2,"I(lag1_unlisted * lag1_eigentuemer2)-lag1_unlisted=0"
...
# Is autocorrelation a concern?
dwtest(omega2 ~  lag1_unlisted + I(lag1_unlisted*lag1_eigentuemer2) 
       + shareF + shareFEW + size_med + size_large + suburban + rurald + rurals
       ,data=data_p)
